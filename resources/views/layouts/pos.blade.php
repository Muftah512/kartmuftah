            ...
            <div class="flex justify-between items-center py-4">
                ...
                <div class="flex items-center space-x-4">
                    <div class="hidden md:block">
                        ...
                        {{-- عرض الدور الأول للمستخدم، أو يمكنك تخصيص هذا --}}
                        @if(auth()->user()->getRoleNames()->isNotEmpty())
                            <span class="bg-blue-500 text-xs px-2 py-1 rounded-full">{{ auth()->user()->getRoleNames()->first() }}</span>
                        @else
                            <span class="bg-gray-500 text-xs px-2 py-1 rounded-full">لا يوجد دور</span>
                        @endif
                    </div>
        ...
        <aside id="sidebar" class="bg-white w-64 min-h-screen shadow-md no-print sidebar transform -translate-x-full md:translate-x-0">
            <div class="p-4 border-b">
                ...
                <div class="mt-4 bg-blue-50 p-3 rounded-lg">
                    ...
                    {{-- الوصول إلى رصيد أول نقطة بيع فقط --}}
                    @if($firstPos)
                        <p class="text-xl font-bold text-green-600">{{ number_format($firstPos->balance) }} ريال</p>
                    @else
                        <p class="text-xl font-bold text-gray-500">0 ريال</p>
                    @endif
                </div>
            </div>
            ...
            <nav class="p-4">
                <ul class="space-y-2">
                    ...
                    <li>
                        <a href="{{ route('pos.transactions') }}" class="flex items-center px-4 py-3 rounded-lg hover:bg-blue-50 text-gray-700 hover:text-blue-600">
                    ...
                    {{-- إضافة عناصر قائمة خاصة بالمدير هنا إذا كنت تريد دمجها في نفس الـ layout --}}
                    {{-- @role('admin')
                        <li>
                            <a href="{{ route('admin.users.index') }}" class="flex items-center px-4 py-3 rounded-lg hover:bg-blue-50 text-gray-700 hover:text-blue-600">
